...
## can do the same using all.fia$TREE_GRM_COMPONENT ; but need to link TRE_CN to the TREE table to get SPCD, etc.
...
### NOTE: this code is modified from Hunter Stanke's excellent `rFIA` package. See the included source code file, as well as `rFIA` documentation.
...
Mapping mortality, recruitment, and change:
```{r}
ggplot() +
  geom_sf(data = states %>% 
            as(.,"sf"),
